...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (err === 'auth_failed') {
      toast.error('Sign-in didn’t complete. Try the email link, or check Google is enabled in Supabase.')
    }
  }, [searchParams])
    ...
    <div className="animate-fade-in">
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm leading-relaxed text-text-secondary">
          Use Google, or we&apos;ll email you a one-tap link — no password.
        </p>
      </div>
        ...
        <>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center">
              <span className="bg-bg-elevated/80 px-3 text-xs font-medium text-text-muted backdrop-blur-sm">or</span>
            </div>
          </div>
        </>
      )}

      <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-brand">Sign in with email</p>
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center">
          <span className="bg-bg-elevated/80 px-3 text-xs font-medium text-text-muted backdrop-blur-sm">new here?</span>
        </div>
      </div>

      <Link
        href="/signup"
        className="btn-secondary flex min-h-[48px] w-full items-center justify-center gap-2 rounded-xl border-white/[0.12] bg-white/[0.04] text-sm font-semibold hover:border-brand/30"
      >
        Sign up with email
      </Link>
    </div>
